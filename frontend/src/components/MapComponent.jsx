...
// Map View Controller
const MapController = ({ center, isSimulating, bearing = 0 }) => {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    const zoom = isSimulating ? 15 : map.getZoom();

    if (isSimulating) {
      // During simulation, jump to position immediately to keep marker locked at center
...
const MapComponent = ({
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      ...
      try {
        ...
        // Robust check for valid numbers
        const isValid = (val) => typeof val === "number" && !isNaN(val);
        if (!isValid(start[0]) || !isValid(start[1]) || !isValid(end[0]) || !isValid(end[1])) {
          console.warn("MapComponent: Invalid coordinates", { start, end });
    ...
    fetchRoute();
  }, [destination, userLocation, showRoute, routeTrigger]);
  ...
  }, [showRoute, destination, userLocation, routeTrigger]);
